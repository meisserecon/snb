...
public class EuroYield extends TimeSeries {
	...
	public EuroYield(String dataFolder, ForexData eur) throws FileNotFoundException, IOException {
		super("Euro Bonds");
		// From https://data.ecb.europa.eu/data/datasets/YC/YC.B.U2.EUR.4F.G_N_A.SV_C_YM.SR_5Y
		List<String> lines = Files.readAllLines(FileSystems.getDefault().getPath(dataFolder, "ECB Data Portal.csv"));
		LocalDate prevTime = null;
		double prevValue = 0.0;
		for (int i = 1; i < lines.size(); i++) {
			String line = lines.get(i);
			StringTokenizer cells = new StringTokenizer(line, ",\"");
			for (int j=0; j<8; j++) cells.nextToken();
